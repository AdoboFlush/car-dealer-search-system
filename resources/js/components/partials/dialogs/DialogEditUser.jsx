...
export function DialogEditUser({ data, successCb = null }) {
    ...
    const form = useForm({
        ...
        roles: data?.roles.map((role) => { return {value: role.name, label: role.name} }),
    });

    const submit = () => {
        form.put(route("user.update", { user: data.id }), {
            preserveScroll: true,
            onSuccess: () => {
                form.reset();
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2">
                        ...
                        <div>
                            <MultiSelect
                                ...
                                values={() => data?.roles.map((role) => { return {value: role.name, label: role.name} })}
